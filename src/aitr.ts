...
export async function explain(what: "word" | "sentence", word: string, sentence: string, lang: Lang, cfg: AiCfg): Promise<string> {
  const task =
    what === "word"
      ...
      : `Explain this ${langName(lang.sl)} sentence: its meaning, then the grammar and phrases a learner may not know (tenses, idioms, word order).`;
  const system = `You help a learner reading ${langName(lang.sl)}. ${task} Answer in ${langName(lang.tl)}, plain text, no markdown, under 80 words.`;
  return (await ask<{ explanation: string }>(cfg, system, `Sentence: ${sentence}`, obj({ explanation: { type: "string" } }))).explanation;
}
